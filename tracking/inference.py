...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        A correct implementation will handle the following special case:
          *  When a ghost is captured by Pacman, all beliefs should be updated
             ...
             captured).
        """
        #estimated manhattan distance between you and the ghost
        noisyDistance = observation
        #this is the sensors input on where the ghost is
        ...
        if noisyDistance == None:
            #edge case, ghost has been eaten
            #set every position except for the jail position
            #to be 0 because we know the ghost is not there,
            #set the jail position to be one because it is for 
            #sure in jail
            ...
            for p in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    #this is the only line that needs to be changed here.
                    #here we are just "summing out" a variable like what
                    #Dr. Ruby said to do in office hours. its important to
                    #note that we are not actually summing out here but
                    #instead replacing our old inference because in these
                    #tests the ghosts movement isnt really being accounted 
                    #for
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        #setup purposes, just like last problem
        allPossible = util.Counter()

        for oldPos in self.legalPositions:
            #get new position distribution like it says to do in the instructions
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            #again taking the instructions suggestion in looping over the
            #distribution and taking a key value pair
            for newPos, prob in newPosDist.items():
                #print "#", newPos, prob
                #just like in the last question we are summing out
                #one of the variables here to create a new probability
                #for this state but this time the ghost is really moving
                #so we cannot just replace the old probability, we must sum
                #into it
                allPossible[newPos] = allPossible[newPos] + (prob * self.beliefs[oldPos])
        #apply all learning that we have done
        allPossible.normalize()
        self.beliefs = allPossible
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        #initializing here, was sure to follow the note and made the representation
        #of my particles a list rather than a dictionary
        particleList = []
        ...
        while numParticles > 0:
            if numParticles < len(self.legalPositions):
                while numParticles >= 0:
                    particleList.append(self.legalPositions[numParticles])
                    numParticles-=1
            else:
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            #first weigh the particles in our belief system, then
            #multiply by the emmission model value for the distance
            #between the square represented by the belief and pacmans 
            #actual position
            for i in range(len(self.particles)): 
            ...
            #this covers the second edge case mentioned about, if all of the
            #particle weights are 0 then we need to make a call to initializeUniformly
    ...
    def getBeliefDistribution(self):
        ...
        #here it is pretty straightforward, we are just adding one for every
        #particle that we had in the initializeUniformly function and then
        #we are normalizing and returning the distribution. I drew inspiration
        #here from the initializeUniformly function in the exactInference class
        
        probabilityDistribution = util.Counter()

        for particle in self.particles:
            probabilityDistribution[particle] += 1
